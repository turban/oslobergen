...
$response = $cartodb->runSql("SELECT cartodb_id, latitude, longitude FROM spot WHERE kartverket IS NULL ORDER BY timestamp DESC LIMIT 10"); 
...
foreach ($response['return']['rows'] as $row) {
	...
	if ($xml->xpath('//ServiceExceptionReport')) {
		exit();
	}
	...
	if ($xml->xpath('//wps:ExecuteResponse/wps:ProcessOutputs')) {		
		...
		if ($altitude !== 'nan') {
			...
			$cartodb->runSql("UPDATE spot SET altitude=$altitude, placename='$placename', ssrid=$ssrid, terrain='$terrain', kartverket=TRUE WHERE cartodb_id = $cartodb_id");
		}		
	} else { // No data found
		$cartodb->runSql("UPDATE spot SET kartverket=TRUE WHERE cartodb_id = $cartodb_id");
	}
}
